...
function dependency_install() {
  ...
  if ! command -v jq; then
    wget -P /usr/bin https://raw.githubusercontent.com/moyada/Xray_onekey/${github_branch}/binary/jq && chmod +x /usr/bin/jq
    judge "安装 jq"
  fi
...
function update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/moyada/Xray_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$(echo -e "$shell_version\n$ol_version" | sort -rV | head -1)" ]]; then
    print_ok "存在新版本，是否更新 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/moyada/Xray_onekey/${github_branch}/install.sh
      print_ok "更新完成"
...
menu() {
  ...
  14)
    if [[ ${shell_mode} == "ws" ]]; then
      read -rp "请输入路径(示例：/moyada/ 要求两侧都包含 /):" WS_PATH
      modify_fallback_ws
      modify_ws
      restart_all
    else
